...
def LaunchYoutube():
    ...
    if(Validate("packageName=com.google.android.youtube processName=com.google.android.youtube","activity")):
        ...
        fileobj_logfile =open(constants.logfile_absolutepath , 'a+')
        fileobj_logfile.write("TC_1" +"\t"+ "TestLaunchYoutube"+"\t"+"FAIL\n")
        LaunchFailCnt+=1
...
def PlayVideo():
    ...
    for i in range(0,2):
        ...
        sleep(5)
    if(Validate("state:started -- attr:AudioAttributes: usage=USAGE_MEDIA content=CONTENT_TYPE_MUSIC","audio")):
        fileobj_logfile =open(constants.logfile_absolutepath , 'a+')
        fileobj_logfile.write("TC_2" +"\t"+ "TestPlayYoutube"+"\t\t"+"PASS\n")
        PlayPassCnt+=1
    else:
        fileobj_logfile =open(constants.logfile_absolutepath , 'a+')
        fileobj_logfile.write("TC_2" +"\t"+ "TestPlayYoutube"+"\t\t"+"FAIL\n")
        PlayFailCnt+=1
    sleep(3)
...
def PauseVideo():
    ...
    sleep(10)
    if(Validate("state:paused -- attr:AudioAttributes: usage=USAGE_MEDIA content=CONTENT_TYPE_MUSIC","audio")):
        fileobj_logfile =open(constants.logfile_absolutepath , 'a+')
        fileobj_logfile.write("TC_3" +"\t"+ "TestPauseYoutube"+"\t"+"PASS\n")
        PausePassCnt+=1
    else:
        fileobj_logfile =open(constants.logfile_absolutepath , 'a+')
        fileobj_logfile.write("TC_3" +"\t"+ "TestPauseYoutube"+"\t"+"FAIL\n")
        PauseFailCnt+=1
    sleep(3)
    KillYoutube()
